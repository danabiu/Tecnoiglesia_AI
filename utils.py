...
def render_custom_css() -> None:
    ...
    st.html("""
            <style>
                ...
                img[alt=image] { width: 300px; display: block; margin: 0 auto; border-radius: 10px; }
                img[alt=Imagen de la Propiedad] { width: 300px; display: block; margin: 0 auto; border-radius: 10px; }
                .block-container {
                    padding-top: 2rem;
...
def remove_links(text: str) -> str:
    """
    Display links and embed images properly

    Args:
    - text (str): The text to process

    Returns:
    - str: The text with links displayed and images embedded
    """
    # Pattern to match Markdown links: [Link text](URL)
    link_pattern = r'\[.*?\]\((.*?)\)'
    # Replace Markdown links with HTML links for non-image URLs
    text = re.sub(link_pattern, lambda match: f'<a href="{match.group(1)}" target="_blank">{match.group(0)}</a>' if not match.group(1).lower().endswith(('.png', '.jpg', '.jpeg', '.gif')) else f'<img src="{match.group(1)}" width="200">', text)
...
class EventHandler(AssistantEventHandler):
    ...
    def on_text_created(self, text) -> None:
        ...
        st.session_state.assistant_text[-1] += "**> 🕵️ BROKIA:** \n\n "
            ...
            if delta.code_interpreter.input:
                ...
                with st.session_state.text_boxes[-1]:
                    ...
                    if f"code_box_{len(st.session_state.text_boxes)}" not in st.session_state:
                        ...
                        st.session_state[f"code_expander_{len(st.session_state.text_boxes)}"] = st.status("**💻 Codigo**", expanded=True)
